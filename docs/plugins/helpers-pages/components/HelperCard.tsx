...
export default function HelperCard({ 
  ...
  const godocUrl = useMemo(() => {
    switch (helper.type) {
      case 'core':
        const baseUrl = 'https://pkg.go.dev/github.com/samber/ro';
        return `${baseUrl}#${functionName}`;
      case 'plugin':
        return 'https://pkg.go.dev/github.com/samber/ro/plugins';
    }
  }, [functionName]);

  const sourceRef = useMemo(() => {
    return `https://github.com/samber/ro/blob/master/${helper.sourceRef}`;
  }, [helper.sourceRef]);
    ...
    <div className="helper-card">
      ...
      <div className="helper-card__prototype">
        <div className="helper-card__prototype-row" ref={prototypeRowRef}>
          ...
          <CodeBlock language="go" className="helper-card__prototype-code">{signatures}</CodeBlock>
        </div>
      </div>
    </div>
  );
...
function SimilarHelpers({ title, similarHelpers, currentCategory, currentType, currentName }: SimilarHelpersProps) {
  ...
  const processedHelpers = similarHelpers
    .map((originalLabel) => {
