...
var pipeline, collector = rootel.Pipe7(
	rootel.CollectorConfig{
		...
		LoggingAttributes: []attribute.KeyValue{
			attribute.String("service", "ee-otel-log"),
			attribute.String("environment", "demo"),
		},
		LogLevelSubscription: otellog.SeverityInfo,
		LogLevelNext:         otellog.SeverityDebug,
		LogLevelError:        otellog.SeverityError,
	},
...
func main() {
	...
	res, err := resource.New(ctx,
		resource.WithAttributes(
			semconv.ServiceName("ee-otel-log"),
	...
	// Initialize OpenTelemetry logger exporter
	exporter, err := stdoutlog.New(stdoutlog.WithPrettyPrint())
	if err != nil {
		log.Fatalf("Failed to create logger exporter: %v", err)
	}

	// Create logger provider with simple processor
	...
	log.Println("Processing completed! Check the console output for OpenTelemetry logs.")
}
